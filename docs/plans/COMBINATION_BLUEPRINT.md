...
本方案基于当前 `local-data/excel/scratch/白细胞正常.xlsx` 表格的结构化规则与翻译链路，提出用于扩展 **异常指标组合**（以 AWBC、SRBC、血常规指标为主）的实施策略。目标是在保持翻译、组合校验、AI 解读彼此解耦的前提下，为医生提供可审核的知识库与自动解读能力。
...
当前表格：`local-data/excel/scratch/白细胞正常.xlsx`（318 行，45 列），每个指标固定 3 列：升高/降低、临床提示、判断依据。
